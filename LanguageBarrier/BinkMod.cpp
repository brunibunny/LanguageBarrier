...
BINK* __stdcall BinkOpenHook(const char* name, uint32_t flags) {
  ...
  if (Config::config().j["fmv"]["useHqAudio"].get<bool>() == true &&
    ...
    if (pVorbis != NULL) {
      std::stringstream logstr;
	  logstr << "Started playing another video with audio redirection configured, "
      ...
      std::string audioPath =
          ...
          Config::fmv().j["audioRedirection"][tmp].get<std::string>();
      ...
      if (pVorbis == NULL) {
        std::stringstream logstr;
		logstr << "Error while trying to open audio redirection at '" << audioPath
